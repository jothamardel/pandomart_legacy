...
import { useTypes, useTypes2 } from '@/framework/type';
...
const GroupsMenu: React.FC<GroupsMenuProps> = ({
  ...
  const selectedMenu =
    groups?.categories.find((type: any) => router.asPath.includes(type.slug)) ?? defaultGroup;
    ...
    >
      ...
      >
        ...
        >
          ...
          >
            {groups?.categories?.map(({ id, name, type, icon }: {id: string; name: string; type: {}; icon: string;}) => (
